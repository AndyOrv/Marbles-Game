...
bool CollisionDetection::RayPlaneIntersection(const Ray&r, const Plane&p, RayCollision& collisions) {                          //plane intersection
...
bool NCL::CollisionDetection::SphereOBBIntersection(const OBBVolume& volumeA, const Transform& worldTransformA, const SphereVolume& volumeB, const Transform& worldTransformB, CollisionInfo& collisionInfo) //obb sphere
{
...
bool CollisionDetection::OBBCapsuleIntersection(const OBBVolume& volumeA, const Transform& worldTransformA,
	...
	float capR = volumeB.GetRadius();
	float capHalfH = volumeB.GetHalfHeight() - capR;
	Quaternion capQ = worldTransformB.GetOrientation();
	Vector3 c_Normal = capQ.ToEuler();
	c_Normal.Normalise();
	Vector3 cTip = Vector3(c_Normal.z * capHalfH, c_Normal.y * capHalfH, c_Normal.x * capHalfH) + worldTransformB.GetPosition();
	Vector3 cBase = -Vector3(c_Normal.z * capHalfH, c_Normal.y * capHalfH, c_Normal.x * capHalfH) + worldTransformB.GetPosition();
	...
	float sphereP = Vector3::Dot(dir, tipDir);     //Then project the sphere’s origin onto our ray direction vector
	Vector3 capPoint; //Center of Sphere within Capsule

	if (sphereP < 0.0f) {
		capPoint = cTip; // point is behind the ray!
	}
	Vector3 point = cTip + (tipDir * sphereP);     //Get closest point on ray line to sphere
	float maxLength = (point - cTip).Length();      //Find length from point to tip of capsule

	if ((capHalfH * 2) <= maxLength)
