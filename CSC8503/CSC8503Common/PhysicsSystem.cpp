...
void PhysicsSystem::ImpulseResolveCollision(GameObject& a, GameObject& b, CollisionDetection::ContactPoint& p) const {
	...
	if (a.isCoin()) {
		...
		gameWorld.DeleteGameObject(&a);
		b.addScore();
	}
	if (b.isCoin()) {
		//std::cout << "Coin hit" << std::endl;
		gameWorld.DeleteGameObject(&b);
		a.addScore();
	}

	if (*(a.getObjType()) == (GameObject::DEATHZONE)) {b.die();}
	if (*(b.getObjType()) == (GameObject::DEATHZONE)) {a.die();}
	if (*(a.getObjType()) == (GameObject::WINZONE)) {b.won();}
	if (*(b.getObjType()) == (GameObject::WINZONE)) {a.won();}

	if (totalMass == 0) {
		return;//two static objects ??
	}
	if (a.nohit || b.nohit) {
		return;//collide but cant be touched
	}
	...
	physA->AddForce(normal * lhs_force_damped);
}
