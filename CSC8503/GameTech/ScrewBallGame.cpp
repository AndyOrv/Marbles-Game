...
ScrewBallGame::ScrewBallGame(GameWorld* gameworld, GameTechRenderer* renderer, PhysicsSystem* physics) : Level(gameworld, renderer, physics) {
	world = gameworld;
...
*/
void ScrewBallGame::InitialiseAssets() {
...
void ScrewBallGame::UpdateGame(float dt) {
	if (!inSelectionMode) {
	...
	renderer->DrawString("V to use Pushers", Vector2(10, 30)); //Draw debug text at 10,20
	renderer->DrawString("C to use Spinners", Vector2(10, 40)); //Draw debug text at 10,20
	...
	if (testPush) {testPush->Update(dt);}
	if (testPush2) {testPush2->Update(dt);}
	...
	if (Window::GetKeyboard()->KeyPressed(KeyboardKeys::C)) {
		spinTest->GetPhysicsObject()->AddTorque(Vector3(0, 5000000, 0));
	}
...
void ScrewBallGame::LockedObjectMovement() {
...
void ScrewBallGame::DebugObjectMovement() {
...
void ScrewBallGame::InitWorld() {//call this function to build a specific level i guess
	world->ClearAndErase();
	physics->Clear();
...
void NCL::CSC8503::ScrewBallGame::InitLevelA()
{
	world->ClearAndErase();
	physics->Clear();
...
void NCL::CSC8503::ScrewBallGame::InitLevelB()
{
	//FLOOR
...
void ScrewBallGame::BridgeConstraintTest() {
...
void NCL::CSC8503::ScrewBallGame::InitAFloor()
{
	//Inital launch area
	...
	AddAngledFloorToWorld(Vector3(40, 55, 100), Vector3(20, 2, 10), 0,0, -45);
	...
	//win zone
	AddWinZoneToWorld(Vector3(-53, -2, 50), Vector3(15, 10, 10));
	AddWinZoneToWorld(Vector3(-147, -2, 50), Vector3(15, 10, 10));
	AddWinZoneToWorld(Vector3(-147, -2, 140), Vector3(15, 10, 10));
	AddWinZoneToWorld(Vector3(-53, -2, 140), Vector3(15, 10, 10));
}
...
GameObject* ScrewBallGame::AddFloorToWorld(const Vector3& position,const Vector3& size) {
...
GameObject* NCL::CSC8503::ScrewBallGame::AddAngledFloorToWorld(const Vector3& position, const Vector3& size, float pitch, float yaw, float roll)
{
...
GameObject* NCL::CSC8503::ScrewBallGame::AddUnityFloorToWorld(const Vector3& position, const Vector3& size)
{
...
*/
GameObject* ScrewBallGame::AddSphereToWorld(const Vector3& position, float radius, float inverseMass) {
...
GameObject* ScrewBallGame::AddCapsuleToWorld(const Vector3& position, float halfHeight, float radius, float inverseMass) {
...
GameObject* ScrewBallGame::AddCubeToWorld(const Vector3& position, Vector3 dimensions, float inverseMass) {
...
GameObject* NCL::CSC8503::ScrewBallGame::AddAngledCubeToWorld(const Vector3& position, const Vector3& dimensions, float pitch, float yaw, float roll, float inverseMass)
{
...
void ScrewBallGame::InitSphereGridWorld(int numRows, int numCols, float rowSpacing, float colSpacing, float radius) {
...
void ScrewBallGame::InitMixedGridWorld(int numRows, int numCols, float rowSpacing, float colSpacing) {
...
void ScrewBallGame::InitCubeGridWorld(int numRows, int numCols, float rowSpacing, float colSpacing, const Vector3& cubeDims) {
...
void ScrewBallGame::InitDefaultFloor() {
	AddAngledFloorToWorld(Vector3(0, -50, 0), Vector3(100, 2, 100), 10,0,0);
	AddFloorToWorld(Vector3(100, -40, 100), Vector3(100, 2, 100));
}

void ScrewBallGame::InitGameExamples() {
	AddPlayerToWorld(Vector3(0, 5, 0));
	AddEnemyToWorld(Vector3(5, 5, 0));
	AddBonusToWorld(Vector3(10, 5, 0));
}

GameObject* ScrewBallGame::AddPlayerToWorld(const Vector3& position) {
	float meshSize = 3.0f;
...
GameObject* ScrewBallGame::AddEnemyToWorld(const Vector3& position) {
	float meshSize = 3.0f;
	float inverseMass = 0.5f;
...
GameObject* ScrewBallGame::AddBonusToWorld(const Vector3& position) {
...
GameObject* ScrewBallGame::AddDeathZoneToWorld(float height)
{
...
GameObject* NCL::CSC8503::ScrewBallGame::AddWinZoneToWorld(const Vector3& position, const Vector3& size)
{
...
PusherObject* ScrewBallGame::AddButtonToWorld(const Vector3& position, const Vector3& position2, float dis, pushDirection x)//side swiper
{
...
StateGameObject* ScrewBallGame::AddStateObjectToWorld(const Vector3& position)
{
...
void NCL::CSC8503::ScrewBallGame::TestMeshPathfinding()
{
...
void NCL::CSC8503::ScrewBallGame::DisplayMeshPathfinding()
{
...
*/
bool ScrewBallGame::SelectObject() {
...
*/
void ScrewBallGame::MoveSelectedObject() {
